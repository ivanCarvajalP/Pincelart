...
async function cargarProductosEnModal(modal) {
    try {
        ...
        if (typeof window.obtenerProductosLocales === 'function') {
            ...
            if (window.firebaseService && window.firebaseService.initialized) {
                console.log('🔥 Sincronizando productos con Firebase...');
                try {
                    // Actualizar cada producto en Firebase
                    for (const producto of productos) {
                        await window.firebaseService.updateProduct(producto.id, { stock: 1 });
                    }
                    console.log('✅ Productos sincronizados con Firebase');
                } catch (error) {
