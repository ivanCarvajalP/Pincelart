...
async function cargarProductosEnModal(modal) {
    try {
        ...
        // SIEMPRE cargar desde carpetas locales (no de Firebase)
        console.log('📁 Obteniendo productos desde carpetas locales...');
        
        if (typeof window.obtenerProductosLocales === 'function') {
            const productosLocales = window.obtenerProductosLocales();
            console.log(`📦 Total en carpetas: ${productosLocales.length}`);
            
            // Crear mapa de productos únicos por imagen
            const mapProductosUnicos = new Map();
            
            for (const producto of productosLocales) {
                // Asegurar que el producto tenga precio y stock FIJO
                if (!producto.precio) producto.precio = 25000;
                producto.stock = 1; // TODOS con stock 1
                
                // Usar la imagen como clave única (una imagen = un producto)
                if (!mapProductosUnicos.has(producto.imagen)) {
                    mapProductosUnicos.set(producto.imagen, producto);
                } else {
                    console.log(`⏭️ Producto duplicado ignorado: ${producto.nombre}`);
                }
            }
            
            productos = Array.from(mapProductosUnicos.values());
            ...
            // SINCRONIZAR CON FIREBASE automáticamente
            if (window.firebaseService && window.firebaseService.initialized) {
                console.log('🔥 Verificando sincronización con Firebase...');
                try {
                    // NO actualizar, solo sincronizar desde Firebase a localStorage
                    const productosFirebase = await window.firebaseService.getAllProducts();
                    if (productosFirebase.success && productosFirebase.data.length > 0) {
                        // Actualizar localStorage con datos de Firebase
                        localStorage.setItem('pincelart_productos', JSON.stringify(productosFirebase.data));
...
async function procesarImagenNueva(imagenFile, productoId) {
    try {
        ...
        if (window.firebaseService && window.firebaseService.initialized) {
            try {
                const url = await window.firebaseService.uploadImage(imagenFile, productoId);
                console.log('✅ Imagen subida a Firebase Storage:', url);
                return url;
            } catch (error) {
                console.log('⚠️ Error subiendo a Firebase Storage, usando Base64:', error);
            }
        }
...
async function actualizarProducto(productoId, form) {
    try {
        ...
        // Obtener productos actualizados de Firebase para sincronizar el catálogo
        if (window.firebaseService && window.firebaseService.initialized) {
            const productosActualizados = await window.firebaseService.getAllProducts();
            if (productosActualizados.success) {
                localStorage.setItem('pincelart_productos', JSON.stringify(productosActualizados.data));
        ...
        // Actualizar la lista de productos en el modal de gestión
        setTimeout(() => {
            ...
            if (modalGestion) {
                ...
                // Recargar productos en el modal de gestión
