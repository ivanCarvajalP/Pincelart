            ...
            productosPorCategoria[categoria].forEach(producto => {
                html += `
                    <div style="border: 1px solid #e0e0e0; border-radius: 10px; padding: 1rem; display: flex; align-items: center; gap: 1rem; transition: all 0.3s ease;">
                        <img src="${producto.imagen}" alt="${producto.nombre}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;" onerror="this.src='images/Logo/logo-pincelart.jpg'">
                        <div style="flex: 1;">
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            <button onclick="editarProducto('${producto.id}')" style="padding: 0.3rem 0.8rem; background: #ff9800; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 0.8rem; transition: all 0.3s ease;">Editar</button>
                            <button onclick="eliminarProducto('${producto.id}')" style="padding: 0.3rem 0.8rem; background: #f44336; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 0.8rem; transition: all 0.3s ease;">Eliminar</button>
                        </div>
                    </div>
                `;
            });
...
async function eliminarProducto(productoId) {
    ...
    const confirmacion = await mostrarConfirmacionCentrada(
    ...
    try {
        // Eliminar de Firebase PRIMERO (prioritario)
        if (window.firebaseService && window.firebaseService.initialized) {
            ...
            if (resultado.success) {
                ...
                console.error('❌ Error eliminando de Firebase:', resultado.error);
            }
        }
        ...
        console.log('✅ Producto eliminado de localStorage');
        
        mostrarMensaje('¡Éxito!', 'Producto eliminado exitosamente de la base de datos.', 'success');
