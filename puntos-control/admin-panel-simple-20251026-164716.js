...
async function actualizarProducto(productoId, form) {
    try {
        ...
        // Actualizar lista en el modal de gestión INMEDIATAMENTE
        const modalGestion = document.querySelector('.modal-productos');
        if (modalGestion) {
            // Guardar filtro activo
            const filtros = modalGestion.querySelectorAll('.filtro-categoria');
            let categoriaActiva = 'todos';
            filtros.forEach(f => {
                const bgColor = window.getComputedStyle(f).backgroundColor;
                if (bgColor === 'rgb(25, 118, 210)' || bgColor === '#1976d2') {
                    categoriaActiva = f.dataset.categoria;
                }
            });
            ...
            if (categoriaActiva === 'todos') {
                ...
                productosParaMostrar = productos.filter(p => p.categoria === categoriaActiva);
            }
            
            // Actualizar arrays globales con los productos actualizados
            productosTodos = productos;
            ...
            setTimeout(() => {
                filtros.forEach(f => {
                    if (f.dataset.categoria === categoriaActiva) {
                        f.style.background = '#1976d2';
                        f.style.color = 'white';
